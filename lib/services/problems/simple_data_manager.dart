...
class UnitGachaAttemptSyncResult {
  ...
  static UnitGachaAttemptSyncResult fromJson(Map<String, dynamic> json) {
    ...
    return UnitGachaAttemptSyncResult(
      ...
      ranAtIso: (json['ranAtIso'] as String?) ?? DateTime.now().toIso8601String(),
    );
  }
}
...
class SimpleDataManager {
  ...
  static const String _unitGachaAttemptQueueKey = '$_namespace/unit_gacha_attempt_events_queue_v1';
  static const String _unitGachaAttemptLastSyncKey = '$_namespace/unit_gacha_attempt_events_last_sync_v1';
  ...
  static Future<int> getPurchaseRecommendAttemptCount(UnitCategory category) async {
  ...
  static Future<int> incrementPurchaseRecommendAttemptCount(UnitCategory category) async {
    final key = _attemptCountKeyFor(category);
    if (key == null) return 0;
    final prefs = await SharedPreferences.getInstance();
  ...
  static ValueListenable<int> get cloudSyncInFlightListenable => _cloudSyncInFlight;
  ...
  static UnitGachaAttemptSyncResult? get lastUnitGachaAttemptSyncResult => _lastUnitGachaAttemptSyncResult;
  ...
  static ValueListenable<int> get learningDataEpochListenable => _learningDataEpoch;
  ...
  static Future<UnitGachaAttemptSyncResult?> loadLastUnitGachaAttemptSyncResult() async {
    final prefs = await SharedPreferences.getInstance();
    ...
    try {
      ...
      if (decoded is Map) {
        final r = UnitGachaAttemptSyncResult.fromJson(Map<String, dynamic>.from(decoded));
  ...
  static UnitGachaAttemptSyncResult? _decodeLastUnitGachaAttemptSyncResult(String? raw) {
    if (raw == null || raw.isEmpty) return null;
    try {
      final decoded = json.decode(raw);
      if (decoded is Map) {
        return UnitGachaAttemptSyncResult.fromJson(Map<String, dynamic>.from(decoded));
      }
    } catch (_) {}
    return null;
  }
  ...
  static Future<void> enqueueUnitGachaAttemptEvent({
    ...
    final eventId = _buildStableAttemptEventId(problemId: problemId, clientTimeIso: clientTime);
  ...
  static Future<UnitGachaAttemptSyncResult> syncUnitGachaAttemptEventsToFirestore() async {
    if (!FirebaseAuthService.isAuthenticated) {
    ...
    if (queue.isEmpty) {
      ...
      final prev = _decodeLastUnitGachaAttemptSyncResult(prefs.getString(_unitGachaAttemptLastSyncKey));
      if (prev != null) {
    ...
    return await _withCloudSyncIndicator(() async {
      ...
      for (final e in queue) {
        ...
        if (eventId == null || problemId == null || status == null || clientTime == null) {
  ...
  static Future<void> _saveLastUnitGachaAttemptSyncResult(
    ...
    try {
      await prefs.setString(_unitGachaAttemptLastSyncKey, json.encode(r.toJson()));
    } catch (_) {}
  }
  ...
  static Future<void> _syncFromFirestore() async {
    try {
      ...
      // SharedPreferencesが更新されたため、メモリキャッシュを破棄してUIへ通知
    ...
    try {
      ...
      await _withCloudSyncIndicator(() async {
        ...
        final aggregationModeKeys = allKeys.where((key) => 
          ...
          !['unit', 'integral', 'limit', 'sequence', 'congruence'].any((type) => 
    ...
    try {
      ...
      await _withCloudSyncIndicator(() async {
        ...
        if (learningKeys.isEmpty) {
          ...
          // 学習記録がなくても、解答イベントだけは溜まっている可能性がある
  ...
  static Future<bool> saveLearningRecord(MathProblem problem, dynamic status) async {
    try {
      ...
      // メモリキャッシュを更新（一覧/集計の即時反映）
      _learningDataCache[problem.id] = Map<String, dynamic>.from(existingData);
      final histAny = existingData['history'];
      if (histAny is List) {
        _learningHistoryCache[problem.id] = histAny
            .whereType<Map>()
            .map((m) => Map<String, dynamic>.from(m))
            .toList();
      } else {
        _learningHistoryCache.remove(problem.id);
      }
  ...
  /// 学習記録の履歴を取得
  /// ローカルデータを優先して即座に返し、バックグラウンドでFirestoreと同期
  static Future<List<Map<String, dynamic>>> getLearningHistory(dynamic problem) async {
    try {
      // まずローカルデータを即座に取得（遅延なし）
  ...
  /// 学習記録の履歴を保存
  static Future<bool> saveLearningHistory(dynamic problem, List<Map<String, dynamic>> history) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final key = '$_namespace/learning/${problem.id}';
  ...
  static Future<bool> saveTutorialLearningHistory(dynamic problem, List<Map<String, dynamic>> history) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final key = '$_namespace/learning/${problem.id}';
      ...
      // 常にSharedPreferencesに保存（チュートリアルは無課金でも保存）
      await prefs.setString(key, json.encode(data));
      _learningDataCache[problem.id] = data;
      _learningHistoryCache[problem.id] = List<Map<String, dynamic>>.from(normalizedHistory);
      _notifyLearningDataChanged();
      print('チュートリアル履歴をSharedPreferencesに保存: problemId=${problem.id}, latestStatus=$latestStatus');
      
      // メモリキャッシュも更新（保存前にgetLearningHistoryが呼ばれて空キャッシュが残るのを防ぐ）
      // ※通常のsaveLearningHistoryと同じ挙動に揃える
      _learningDataCache[problem.id] = data;
      _learningHistoryCache[problem.id] = List<Map<String, dynamic>>.from(normalizedHistory);
      ...
      if (FirebaseAuthService.isAuthenticated) {
        ...
        if (userId != null) {
          try {
            ...
            if (success) {
              print('Successfully saved tutorial learning history to Firestore for problem ${problem.id}');
            } else {
              print('Warning: Failed to save tutorial learning history to Firestore for problem ${problem.id}');
            }
          } catch (e, stackTrace) {
            print('Error saving tutorial history to Firestore (continuing with local save): $e');
            print('Stack trace: $stackTrace');
          }
        } else {
          print('チュートリアル履歴保存: userIdがnullのためFirestore保存をスキップ');
        }
      } else {
        print('チュートリアル履歴保存: 未認証のためFirestore保存をスキップ（ローカルのみ保存）');
      }
      
      return true;
    } catch (e) {
      print('Error saving tutorial learning history: $e');
      return false;
    }
  }
  ...
  static Future<bool> saveSelectedFreeGachas(List<String> prefsPrefixes) async {
    try {
      if (prefsPrefixes.length > 2) {
        print('Warning: More than 2 gachas selected. Only first 2 will be saved.');
