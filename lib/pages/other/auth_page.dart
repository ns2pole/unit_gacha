...
class _AuthPageState extends State<AuthPage> {
  ...
  Future<_EmailLockoutState> _loadEmailLockoutState(
    ...
    final lockedUntilEpochMs =
        prefs.getInt(_lockoutPrefsKey(emailNormalized, 'lockedUntilEpochMs'));
    final requireReset =
  ...
  Future<void> _signInWithEmailAndPassword() async {
    ...
    try {
      ...
      String errorMsg =
          _t('エラー: ${e.message ?? e.code}', 'Error: ${e.message ?? e.code}');
    ...
    try {
      ...
      if (userCredential != null) {
        ...
        if (mounted) {
          // 成功メッセージを表示
          final loginMethod =
              FirebaseAuthService.loginMethod ?? _t('Googleアカウント', 'Google account');
          final message = _isSignUp
              ? _t('$loginMethodで新規登録しました', 'Signed up with $loginMethod')
              : _t('$loginMethodでクラウドに保存しました', 'Saved to cloud with $loginMethod');
          ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      ...
      if (userCredential != null) {
        ...
        if (mounted) {
          // 成功メッセージを表示
          final loginMethod =
              FirebaseAuthService.loginMethod ?? _t('Appleアカウント', 'Apple account');
          final message = _isSignUp
              ? _t('$loginMethodで新規登録しました', 'Signed up with $loginMethod')
              : _t('$loginMethodでクラウドに保存しました', 'Saved to cloud with $loginMethod');
          ScaffoldMessenger.of(context).showSnackBar(
                      ...
                      if (!_isPasswordResetMode) ...[
                        TextFormField(
                          ...
                          validator: (value) {
                            ...
                            final minLen =
                                _isSignUp ? _minPasswordLengthSignUp : _minPasswordLengthSignIn;
                        ...
                        ElevatedButton(
                          ...
                          child: _loadingButtonChild(
                            label: _isSignUp ? _t('新規登録', 'Sign up') : _t('ログイン', 'Sign in'),
                        ...
                        ElevatedButton(
                          ...
                          child: _loadingButtonChild(
                            ...
                            loading: _isBusy(_AuthBusyAction.passwordReset),
                            spinnerColor: Colors.white,
                          ),
                        ),
                        const SizedBox(height: 8),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TextButton(
                          ...
                          child: Text(_t('電話番号を変更', 'Change phone number')),
                        ),
                      ],
                    ),
                  ),
